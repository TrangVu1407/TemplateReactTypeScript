...
import type { PropsGetProductColor } from "services/product_color/product_color";
...
const Size: React.FC<Props> = ({ open, closeOpen, product_type_name, product_type_id }) => {
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: 'name',
            headerName: `${t("product_color_name")}`,
            width: 150,
        ...
        {
            field: 'describe',
            headerName: `${t("product_color_describe")}`,
            width: 230,
            // không cho thay đổi giá trị bảng
            editable: false,
        },
        {
            field: 'notes',
            headerName: `${t('product_color_notes')}`,
            width: 270,
            // không cho thay đổi giá trị bảng
            editable: false,
        },
    ...
    const openInfoDialog = async () => {
        setType(false);
        setItemUpdate({ name: "Màu xanh", describe: "Màu xanh", notes: "", id: 0, product_type_id: product_type_id });
        setOpenDialog(true);
    };
    const closeOpenDialog = async (callApi: boolean) => {
        if (callApi) {
            await getProductColor();
            setOpenMessage(true);
        }
        setOpenDialog(false);
    };
    ...
    const getProductColor = async () => {
        try {
            ...
            let params: PropsGetProductColor = { shop_id: data.employee.shop_id, product_type_id: product_type_id };
            const response = await productColorServices.getList(params);
