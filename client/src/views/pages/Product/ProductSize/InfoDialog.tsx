...
import type { PropsCreateProductSize, PropsUpdateProductSize } from "services/product_size/product_size"
...
const InfoDialog: React.FC<Props> = ({ open, closeOpenDialog, type, item, setMessage }) => {
    ...
    async function createTypeProduct() {
        try {
            ...
            if (result && !result.error) {
                setMessage({ notification: `${t('product_size_create_success')}`, severity: "info" });
                let callApi = true;
                closeOpenDialog(callApi);
            } else {
                switch (result.code) {
                    case 209:
                        setIsExistingMessage(`${t('product_size_create_error_is_existing')}`);
                        setIsExisting(true);
                        break;
                    default:
                        // code block
    ...
    const updateTypeProduct = async () => {
        try {
            ...
            let body: PropsUpdateProductSize = {
                shop_id: data.employee.shop_id,
                id: item.id,
                product_type_id: item.product_type_id,
                name: value.name,
            ...
            const result = response.data;
            if (result && !result.error) {
                setMessage({ notification: `${t('product_size_update_success')}`, severity: "success" });
                let callApi = true;
                closeOpenDialog(callApi);
            } else {
                switch (result.code) {
                    case 209:
                        setIsExistingMessage(`${t('product_size_update_error_is_existing')}`);
                        setIsExisting(true);
                        break;
                    default:
                        // code block
